...
| GPU inference profile | `INFERENCE_BACKEND=openai_compatible` exists; a sized vLLM deployment in Compose prod does not. Publish latency/concurrency numbers and wire `scripts/eval_live.py` into the release process as the gate it was built to be |
